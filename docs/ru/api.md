[← Workflow](workflow.md) · [Назад к README](../../README.md) · [Конфигурация →](configuration.md)
...
- Валидный ввод переводит на следующий недостающий шаг или сразу к confirm
- Mixed input может заполнить несколько полей за одно сообщение, но только после детерминированной backend-проверки
- Невалидный ввод оставляет на текущем шаге и возвращает retry-подсказку с deterministic fallback при неудачном ответе модели
- На confirm `да` сохраняет лид и возвращает подтверждение с `lead_id`
- На confirm `нет` сбрасывает flow обратно на `name`
- На confirm фразы вроде `исправь имя`, `исправь контакт`, `исправь задачу` возвращают только к нужному полю
